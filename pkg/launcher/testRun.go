...
func readTestRunFromJsonFile(
	...
	if err != nil {
		...
		if err != nil {
			...
			if !isExists {
				log.Printf("readTestRunFromJsonFile file '%s' does not exist. url: 's'", jsonFilePath, url.Path)
			} else {
				...
				if err != nil {
					...
					if len(jsonContent) <= 0 {
						...
						if err != nil {
							log.Printf("readTestRunFromJsonFile file '%s' could not be parsed from json. status could not be read.", url.Path)
						} else {
